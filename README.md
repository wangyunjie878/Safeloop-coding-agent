...
### 方式一：像 opencode CLI 一样进入项目目录后对话

先进入你要让 agent 操作的代码目录：
...
如果需要项目级配置，例如自定义测试命令、阻止某些危险命令、调整超时或默认模型，可以在项目中放 `safeloop.yml`，然后运行：
...
给老师展示核心机制时，可以使用 mock LLM，不需要真实 API key：
...
真实 LLM key 由使用者自己提供，仓库内不包含任何真实凭据。推荐把 DeepSeek key 写入操作系统 keyring：
