...
本项目的交付范围是课程可验收的简化版本地 coding agent，而不是复刻完整的 Codex、Claude Code 或商业 IDE agent。SafeLoop 第一版只承诺完成一个小型但完整的闭环：读取允许范围内的代码、写入局部修改、执行受控命令、运行测试、把失败反馈交给下一轮决策，并在 mock LLM 下确定性展示一次自我修正。

第一版明确不做以下内容：大型代码索引、语言专用 AST 分析、复杂交互式终端、无限多轮自动修复、完整 IDE 集成、多人协作权限系统、云端多租户服务，以及对任意真实项目依赖环境的自动安装。WebUI 和 DeepSeek provider 都保持最小可用，用来展示 harness 机制和满足分发/凭据要求，而不是做完整产品化体验。
